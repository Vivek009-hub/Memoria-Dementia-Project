/**
 * DeleteMemoryDialog.jsx — Accessible Delete Confirmation Modal
 */

import React, { useState } from 'react';
import { AlertTriangle, Trash2, X } from 'lucide-react';

export function DeleteMemoryDialog({ memory, isOpen, onClose, onConfirmDelete }) {
  ...
  const handleConfirm = async () => {
    ...
    setErrorMsg('');
    try {
      await onConfirmDelete(memory._id);
      onClose();
    } catch (err) {
      setErrorMsg(err.message || 'Failed to delete memory. Please try again.');
    } finally {
      setDeleting(false);
    }
  };
    ...
    >
      <div className="bg-slate-900 border border-red-500/30 rounded-3xl w-full max-w-md overflow-hidden shadow-2xl p-6 space-y-5 animate-in fade-in zoom-in-95">
        <div className="flex items-center space-x-3 text-red-400">
          ...
          <div>
            <h2 id="delete-dialog-title" className="text-xl font-black text-white">
              Delete Memory?
            </h2>
        ...
        <div className="bg-slate-950 p-4 rounded-2xl border border-slate-800 text-slate-300 text-sm">
          Are you sure you want to delete <span className="font-bold text-white">"{memory.title}"</span> from your memory collection?
        </div>
