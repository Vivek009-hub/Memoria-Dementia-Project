...
export function NotificationsPage({ onNavigate, onUnreadCountChange }) {
  ...
  const fetchNotifications = useCallback(async () => {
    ...
    try {
      ...
      setErrorMsg(
        err.message || 'Could not load notifications right now.'
      );
    } finally {
      setLoading(false);
    }
  ...
  const handleNotificationClick = (notification) => {
    ...
    if (notification.type === 'REMINDER' && onNavigate) {
      ...
      notification.type === 'COMMUNITY_SESSION' &&
      onNavigate
    ) {
      onNavigate('/app/community');
    } else if (notification.type === 'MEETING' && onNavigate) {
      ...
      (notification.type === 'SOS' ||
        notification.type === 'POSSIBLE_FALL') &&
      onNavigate
    ) {
      onNavigate('/app/safety');
    }
        ...
        <div className="flex items-center space-x-2 overflow-x-auto pb-1 scrollbar-none">
          {TYPE_FILTERS.map((cat) => (
            <button
              ...
              className={`py-2 px-4 rounded-xl text-sm font-extrabold transition-all whitespace-nowrap ${selectedType === cat.id
                ? 'bg-blue-600 text-white shadow-md'
                : 'bg-slate-100 text-slate-700 hover:bg-slate-200 border border-slate-200'
                }`}
            >
