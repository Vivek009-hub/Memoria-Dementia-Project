...
export function MeetingCircleRoomModal({ session, isOpen, onClose }) {
  ...
  const joinRoom = async () => {
    ...
    setErrorMsg('');
    try {
      const res = await meetingsApi.joinMeeting(session._id);
      if (res.data) {
  ...
  const handleLeaveRoom = async () => {
    try {
      if (session?._id) {
        await meetingsApi.leaveMeeting(session._id);
      }
    } catch {
      // Ignore leave error
    } finally {
        ...
        <div className="p-6 space-y-6 overflow-y-auto flex-1 flex flex-col items-center justify-center text-center">
          {loading ? (
            ...
            <div className="w-full space-y-6">
              {/* Simulated Video Stage Container */}
              ...
              {/* Room Credential Token Metadata (Safe View) */}
